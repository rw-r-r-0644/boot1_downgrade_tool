...
void SRAM_TEXT __attribute__((__noreturn__)) smc_shutdown(bool reset)
{
    ...
    if(reset) {
        rtc_write(RTC_CONTROL1, SLEEP_EN | DEVPWR_EN | COUNT4_EN);
        rtc_write(RTC_OFFTMR, 0);
        clear32(LT_RESETS, 1);
    } else {
        rtc_write(RTC_OFFTMR, 0);
        rtc_write(RTC_CONTROL1, SLEEP_EN | COUNT4_EN);
        rtc_write(RTC_CONTROL1, SLEEP_EN | COUNT4_EN | POFF_EXE);
    }
